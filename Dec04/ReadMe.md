...
The automatic passport scanners are slow because they're having trouble __detecting which passports have all required fields__. The expected fields are as follows:
...
The fourth passport is missing two fields, `cid` and `byr`. Missing `cid` is fine, but missing any other field is not, so this passport is __invalid__.
...
Count the number of __valid__ passports - those that have all required fields __and valid values__. Continue to treat `cid` as optional. __In your batch file, how many passports are valid?__
